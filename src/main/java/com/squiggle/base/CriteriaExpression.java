...
public class CriteriaExpression extends Criteria {
  ...
  /**
   * Initializes a CriteriaExpression with an initial criteria term, an operator,
   * and a another trailing criteria expression. Note that this constructor
   * definition is recursive and represents the rule
   * 
   ...
   *                   {@link Operator#OR OR}, that joins the initial criteria
   *                   with the trailing criteria expression.
   * 
  ...
  /**
   * Recursively generates a CriteriaExpression from a given list of criteria and
   * an infix operator to join each criteria with the next in the list.
   * 
  ...
  /**
   * Returns the operator, either {@link Operator#AND AND} or {@link Operator#OR
   * OR}, in this criteria expression that joins the initial criteria with the
   * trailing criteria expression.
   * 
   * @return the operator, either {@link Operator#AND AND} or {@link Operator#OR
   *         OR}, in this criteria expression.
   */
  ...
  /**
   * Writes this criteria expression to the given output destination. If both term
   * and expression are null, this method generates no output. If expression is
   * null, but term is defined, the method writes the term criteria to the output.
   * If both expression and term are defined, however, the method creates a new
   * logic operator object, either {@link Operator#AND AND} or {@link Operator#OR
   * OR}, assigns it the term and expression, and writes this new operator to the
   * output.
   * 
   * @param out the output destination to which to write this criteria expression.
   * 
